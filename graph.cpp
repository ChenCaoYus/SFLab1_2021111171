...
string TextGraph::randomWalk() {
    ...
    next = words[index];
    while (true) {
        cout << "Current node: " << next << endl; // 输出当前节点
        cout << "Do you want to continue walking? (yes/no): ";
        string userInput;
        cin >> userInput; // 获取用户输入
        if (userInput == "no") {
            break; // 用户选择停止游走
        }
        ...
        if (find(edges.begin(), edges.end(), make_pair(word, next)) != edges.end()) {
    ...
    path += (edges.empty() ? word : edges.back().second);

    // 输出整个游走路径
    cout << "Random walk path: " << path << endl;
